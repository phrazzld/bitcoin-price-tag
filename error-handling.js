...
export function logError(error, context = {}) {
  ...
  if (useRichContext) {
    ...
    if (
      ...
      errorType === ErrorTypes.EXTENSION ||
      errorType === ErrorTypes.CONTEXT ||
      errorType === ErrorTypes.CSP ||
      severity === ErrorSeverity.CRITICAL
    ) {
      try {
    ...
    if (
      errorType === ErrorTypes.RUNTIME ||
      errorType === ErrorTypes.EXTENSION ||
      errorType === ErrorTypes.BRIDGE ||
      errorType === ErrorTypes.INTEGRITY
    ) {
      try {
    ...
    if (errorType === ErrorTypes.AMAZON && logSettings.amazonDetailLevel !== 'none') {
      try {
  ...
  if (
    logSettings.logToStorage &&
    (logSettings.diagnosticMode || severity === ErrorSeverity.CRITICAL)
  ) {
...
function sanitizeContextForLogging(context) {
  ...
  for (const [key, value] of Object.entries(context)) {
    if (value === null || value === undefined) {
      ...
      result[key] = {
        ...
        headers: Object.fromEntries(
          ...
          typeof value.headers?.entries === 'function' ? [...value.headers.entries()] : [],
        ),
        type: value.type,
      };
    } else if (typeof Node !== 'undefined' && value instanceof Node) {
      // Handle DOM nodes specially - only if Node is defined
      try {
        result[key] = {
    ...
    } else if (value && typeof value === 'object' && typeof value.nodeType === 'number') {
      // Fallback for environments where Node isn't defined but we have a node-like object
      try {
        result[key] = {
          nodeType: value.nodeType,
        ...
        result[key] = `[Node-like object access error: ${nodeError.message}]`;
      }
...
export function categorizeError(error) {
  ...
  // WeakSet issues
  if (error.message.includes('WeakSet') || error.message.includes('processedNodes')) {
  ...
  if (error.message.includes('chrome.storage') || error.message.includes('Storage')) {
  ...
  if (error.message.includes('amazon') || error.message.includes('Amazon')) {
...
export function logPerformance(stats, operation, severity = ErrorSeverity.DEBUG) {
  ...
  const message = `${logSettings.prefix} [Performance] ${operation}: ${
    stats.startTime && stats.endTime ? `${perfContext.duration}ms` : JSON.stringify(stats)
  }`;
