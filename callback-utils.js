  ...
  return function safeCallbackWrapper(...args) {
      ...
      if ('fallback' in options) {
        ...
        try {
          ...
          if (trackTiming && fallbackStartTime) {
            ...
            if (!silent && fallbackTime > 50) { // Only log slow fallbacks
              console.debug(`Bitcoin Price Tag: Fallback execution time: ${fallbackTime}ms for ${context}`);
            }
          }
    ...
    try {
      ...
      if (trackTiming && startTime) {
        ...
        if (!silent && executionTime > 100) { // Only log slow callbacks
          console.debug(`Bitcoin Price Tag: Callback execution time: ${executionTime}ms for ${context}`);
        }
      }
