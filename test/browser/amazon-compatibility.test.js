...
test.describe('Bitcoin Price Tag - Amazon Page Compatibility', () => {
    ...
    await page.evaluate(() => {
      ...
      setTimeout(() => {
        ...
        if (subtotalElement) {
          ...
          const conversionNode = subtotalElement.parentNode.querySelector('.btc-price-converted-amazon');
        ...
        try {
          ...
          if (restrictedIframe && restrictedIframe.contentDocument) {
            ...
            const isInRestrictedIframe = () => {
              ...
              return restrictedIframe.hasAttribute('sandbox');
            };
          ...
          console.log('Expected error accessing restricted iframe (good):', restrictedError.message);
        }
      } catch (iframeError) {
  ...
  test('should gracefully handle content script errors', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      const createError = window.createError || function(message, type, details = {}) {
      ...
      const logError = window.logError || function(error, context = {}) {
    ...
    const errors = await page.evaluate(() => {
      return window.testErrorsLogged || [];
    });
