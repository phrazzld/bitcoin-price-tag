...
export function shouldRefreshCache(cachedData) {
  ...
  const freshness = cachedData.freshness || determineCacheFreshness(cachedData.timestamp);
...
export function calculatePriceVolatility(newData, oldData) {
  ...
  const percentChange = Math.abs((newData.btcPrice - oldData.btcPrice) / oldData.btcPrice);
