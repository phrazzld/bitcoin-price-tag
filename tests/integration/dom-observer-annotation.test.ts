...
describe('DomObserver and DOM Annotation Integration', () => {
  ...
  it('should pass the same processedNodes set to the annotation function', () => {
    // Create a mock annotation function to verify the parameters
    const mockAnnotationFunction = vi.fn();
    
    // Create the DOM observer with our mock function
    const domObserver = createDomObserver(
      document.body,
    ...
    // We can't easily test MutationObserver in JSDOM, but we can verify
    // that the parameters are passed correctly to the annotation function
    // by checking that the observer was created successfully
  ...
  it('should filter out script and style elements properly', () => {
    ...
    const trackingAnnotationFunction = (node: Node, priceData: PriceData, processedNodes: Set<Node>) => {
