...
function checkExecutionContext() {
  ...
  try {
    ...
    try {
      result.details.hasExtensionApi =
    ...
    try {
      ...
      if (result.hasBridge) {
        ...
        if (window.bitcoinPriceTagBridge._bridgeStatus) {
          ...
          if (
            window.bitcoinPriceTagBridge._bridgeStatus.consecutiveErrors >=
    ...
    if (!bridgeValidation.available) {
      ...
      if (bridgeValidation.hasBridge && bridgeValidation.hasGetFallbackData) {
        try {
          ...
          console.debug(
            'Bitcoin Price Tag: Error getting bridge fallback data',
    ...
    try {
      ...
      if (bridgeValidation.hasBridge && bridgeValidation.hasGetFallbackData) {
        try {
          ...
          console.debug(
            'Bitcoin Price Tag: Error getting bridge fallback data after error',
            fallbackError.message,
          );
        }
      }
  ...
  const processPage = async (priceData) => {
    try {
      ...
      if (
        !priceData ||
        !priceData.btcPrice ||
        isNaN(priceData.btcPrice) ||
        priceData.btcPrice <= 0
      ) {
        throw createError('Invalid price data', ErrorTypes.EXTENSION, { receivedData: priceData });
      }
