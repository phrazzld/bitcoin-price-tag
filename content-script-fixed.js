  ...
  try {
    ...
    try {
      ...
      safeCallbackUtils = {
        ...
        safeCallback: (callback, options = {}) => {
          ...
          return (...args) => {
            if (typeof callback !== 'function') {
              if (!silent) {
                console.debug(
                  'Bitcoin Price Tag: Non-function callback received (' +
                    typeof callback +
                    ') in ' +
                    context,
                );
            ...
            try {
              ...
              if (!silent) {
                console.debug(
                  'Bitcoin Price Tag: Error in callback execution in ' + context + ':',
                  error.message,
                );
              }
              if ('fallback' in options) {
        ...
        safeExecute: (callback, args = [], options = {}) =>
          safeCallbackUtils.safeCallback(callback, options)(...args),
        ...
        safeChromeCallback: (callback, options = {}) => {
          ...
          return (response) => {
            if (chrome?.runtime?.lastError) {
              console.debug(
                'Bitcoin Price Tag: Chrome runtime error:',
              ...
              if (typeof callback === 'function') {
                try {
                  ...
                  console.debug(
                    'Bitcoin Price Tag: Error in chrome callback error handler:',
    ...
    window.bitcoinPriceTagBridge = {
      ...
      checkBridgeHealth: () => {
        ...
        try {
          ...
          status.sendMessage = status.runtime && typeof chrome.runtime.sendMessage === 'function';
          ...
          if (status.runtime) {
            try {
              ...
              const _extensionId = chrome.runtime.id;
              const _extensionUrl = chrome.runtime.getURL('');
              ...
              safeCallbackUtils.safeChromeCallback(
                (response) => {
                  ...
                  try {
                    ...
                    console.debug('Bitcoin Price Tag: Error caching price data', {
      ...
      isExtensionContextAvailable: () => {
        ...
        try {
          ...
          result.hasSendMessage =
            result.hasRuntime && typeof chrome.runtime.sendMessage === 'function';
          ...
          try {
            ...
            if (result.isAmazonFrame) {
              ...
              if (document.body) {
                result.details.hasPopoverClass =
                ...
                if (
                  ...
                  result.restrictionReason =
                    result.restrictionReason || 'amazon_restricted_content';
                }
              }
              ...
              try {
                ...
                result.details.isSmallFrame =
                  width < smallFrameThreshold || height < smallFrameThreshold;
          ...
          console.warn(
            'Bitcoin Price Tag: Error in context detection, assuming restricted',
            e.message,
          );
          return {
            available: false,
    ...
    try {
      ...
      if (contextState.isRestricted || contextState.isAmazonFrame) {
        ...
        // Commented out as configureLogging is not defined
        /*
    ...
    if (contextState.isAmazonFrame && contextState.isRestricted) {
      console.warn(
        `Bitcoin Price Tag: Exiting early in restricted Amazon frame - ${contextState.restrictionReason}`,
      );
    ...
    if (contextState.available && !hasPotentialCSPIssues) {
      ...
      try {
        ...
        moduleScript.onerror = (event) => {
          ...
          console.warn('Bitcoin Price Tag: Failed to load bootstrap module', {
            ...
            errorDetails: event && event.message ? event.message : 'no details available',
            timestamp: new Date().toISOString(),
          });
      ...
      console.warn(
        `Bitcoin Price Tag: Skipping module loading in restricted context - ${contextState.restrictionReason}`,
      );
    }
      ...
      try {
        ...
        if (cspMeta) {
          ...
          if (cspContent.includes('script-src') && !cspContent.includes('unsafe-inline')) {
            console.warn(
              'Bitcoin Price Tag: CSP blocks inline scripts, trying non-inline approach',
            );
        ...
        fallbackScript.onerror = (event) => {
          console.error('Bitcoin Price Tag: Fallback script load error', {
            error: event && event.message ? event.message : 'unknown script error',
            type: event.type,
          });
        };
        ...
        try {
          ...
          scriptEl.onerror = (event) => {
            console.error('Bitcoin Price Tag: Non-inline script load error', {
              error: event && event.message ? event.message : 'unknown error',
              url: scriptUrl,
            });
          };
